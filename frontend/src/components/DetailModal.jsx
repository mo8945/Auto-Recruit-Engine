...
const DetailModal = ({ applicant, onClose, onRefresh }) => {
  ...
  const resume = applicant.resumes?.[0] || null;
      ...
      <div className="bg-slate-900 border border-slate-800 w-full max-w-4xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-200">
        ...
        <div className="p-10 space-y-10 max-h-[70vh] overflow-y-auto text-left">
          <div className="flex items-center gap-8">
            <div className="w-24 h-24 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl flex items-center justify-center text-4xl font-black text-white shadow-xl shadow-blue-500/20 shrink-0">
              {applicant.name?.[0]}
            </div>
            <div>
              <div className="flex items-center gap-4 mb-2">
                <h3 className="text-4xl font-black text-white tracking-tight">{applicant.name}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-4">
              ...
              <div className="bg-slate-950/80 p-8 rounded-[2rem] border border-slate-800 shadow-inner min-h-[180px] flex items-center">
                <p className="text-slate-200 leading-relaxed text-lg font-medium italic">
                  {resume?.summary_text ? `"${resume.summary_text}"` : "심층 분석 데이터를 불러오는 중입니다..."}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h4 className="text-slate-500 font-black text-xs uppercase tracking-widest flex items-center gap-2">
                <Award size={16} className="text-amber-500" /> 역량 키워드 분류
              </h4>
              <div className="flex flex-wrap gap-2 pt-2">
                {keywords.map((kw, idx) => (
                  <div key={idx} className={`px-4 py-2 rounded-2xl border text-sm font-bold flex items-center gap-2 transition-all hover:scale-105 ${
        ...
        <div className="p-8 bg-slate-900 border-t border-slate-800 flex justify-between items-center">
          <div className="flex gap-4">
            <button 
              ...
              className="px-6 py-3 bg-amber-600/10 hover:bg-amber-600 text-amber-500 hover:text-white border border-amber-600/20 rounded-2xl font-black transition-all flex items-center gap-2"
            >
              <ArrowRight size={20}/> 면접 예정
            </button>
            <button 
              disabled={isUpdating}
              onClick={() => handleStatusUpdate("최종 합격")} 
              className="px-6 py-3 bg-green-600/10 hover:bg-green-600 text-green-500 hover:text-white border border-green-600/20 rounded-2xl font-black transition-all flex items-center gap-2"
            >
              <UserCheck size={20}/> 합격 처리
            </button>
            <button 
              disabled={isUpdating}
              onClick={() => handleStatusUpdate("불합격")} 
              className="px-6 py-3 bg-red-600/10 hover:bg-red-600 text-red-500 hover:text-white border border-red-600/20 rounded-2xl font-black transition-all flex items-center gap-2"
            >
              <UserX size={20}/> 불합격 처리
            </button>
          </div>
