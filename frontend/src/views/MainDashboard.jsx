  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        {/* 상단 접속자 정보 표시 */}
        <div className="mb-6 flex justify-between items-center text-sm text-slate-500 bg-slate-900/30 p-4 rounded-2xl border border-slate-800">
           <p>접속 계정: <span className="text-blue-400 font-bold">{user.email}</span></p>
           <p>담당 부서: <span className="text-amber-400 font-bold">[{profile?.department || "미지정"}]</span></p>
        </div>
        ...
        {view === "list" ? (
          <>
            ...
            <div className="mt-8 mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div>
                ...
                <p className="text-slate-500 text-sm">부서별 최적화 채용 관리 시스템입니다. 🐧</p>
              </div>
          ...
          <VisualizationView applicants={applicants} />
        )}
      </div>
      ...
      {selectedApplicant && (
        <DetailModal 
          ...
          onRefresh={loadData}
          userProfile={profile} // 메모 작성을 위해 부서 정보 전달
        />
      )}
    </div>
  );
